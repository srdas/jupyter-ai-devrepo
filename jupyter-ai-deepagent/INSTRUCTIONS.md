Using the `jupyter-ai-jupyternaut` submodule as a template, create another persona called `jupyter-ai-deepagent` in its own submodule that implements the langchain deepagents (in https://github.com/langchain-ai/deepagents). 
...
4. Make the deepagent interactive so that it prompts for 
...
jupyter-ai-deepagent is installed and ready. After running
  just start (or uv run jupyter lab), you can use it:

  1. Open the Jupyter AI chat panel in the left sidebar
  2. Type @DeepAgent to mention the persona — you'll see the
  help menu
  3. Type /deepagent create to interactively build your first
   agent
  ...
  The deepagent entry point is registered alongside
...
New usage (after restarting JupyterLab):                                                                                      
  ...
  ├────────────────────────────────────┼─────────────────────────┤                                                              
  │ @DeepAgent /deepagent create       │ @DeepAgent create       │                                                              
  ├────────────────────────────────────┼─────────────────────────┤                                                              
  │ @DeepAgent /deepagent list         │ @DeepAgent list         │                                                              
  ├────────────────────────────────────┼─────────────────────────┤                                                              
  │ @DeepAgent /deepagent use My Agent │ @DeepAgent use My Agent │                                                              
  └────────────────────────────────────┴─────────────────────────┘                                                              
## Example Agent

Agent NewsAgent saved and activated!

Saved to: /Users/sanjivda/Library/Jupyter/jupyter_ai/deepagents/newsagent.json

You can now send messages and I'll process them with this agent.

You can also invoke it later with <span class="jp-chat-mention">@DeepAgent</span> use NewsAgent.
